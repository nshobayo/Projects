...
public class State implements Comparable<Object> {
    ...
    public void initializeChildren(){
    	...
    	Move[] pMoves = board.getPossibleMoves(player);
    	State[] states = new State[pMoves.length];
    	
    	//creates new children states with player's opponent as player and a possible move.
    	for( int i = 0; i < pMoves.length;i++){
    		states[i]=new State(player.opponent(),new Board(board,pMoves[i]), pMoves[i]);
    	}	
